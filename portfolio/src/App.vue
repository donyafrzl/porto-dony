  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
    ...
    <nav class="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md z-50 border-b border-slate-200">
      <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        <div class="font-bold text-xl text-slate-800">{{ personal.name }}</div>
        <button @click="menuOpen = !menuOpen" class="md:hidden p-2 text-slate-600 hover:text-slate-900">
      ...
      <!-- Mobile menu -->
      <div v-if="menuOpen" class="md:hidden bg-white border-t border-slate-200 px-4 py-2">
        <button
          v-for="item in navItems"
          :key="item.id"
          @click="scrollTo(item.id)"
    ...
    <Education :educations="educations" :organizations="organizations" />

    <!-- Footer -->
    <footer class="py-12 px-4 bg-slate-900 text-white">
      <div class="max-w-6xl mx-auto text-center">
        <div class="text-2xl font-bold mb-4">{{ personal.name }}</div>
        <p class="text-slate-400 mb-6">{{ personal.title }}</p>
        <div class="flex justify-center gap-6 mb-8">
          <a :href="personal.linkedin" target="_blank" class="text-slate-400 hover:text-white transition-colors">
